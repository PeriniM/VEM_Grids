...
% verifica se il dominio è tutto riempito
if rem(suddy,2)==0
    y_centro_ultimo=y_centro-(a*suddy+r_vertice*(suddy-2));
    if y_centro_ultimo-r_vertice>ymin && (y_centro_ultimo-r_vertice-tan(alpha/2)*d/2)>=ymin
        suddy=suddy+1;
    end
else
    y_centro_ultimo=y_centro-(a*(suddy-1)+r_vertice*(suddy-1));
    if y_centro_ultimo-a>ymin && (y_centro_ultimo-2*a-r_vertice+tan(alpha/2)*d/2)>=ymin
        suddy=suddy+1;
    end
end
...
for i=1:suddy
    for j=1:suddx       
        ...
        for k=1:n       
           x(k)=(x_centro-xmin)+r_vertice*cos(alpha*(k-1)+theta);
           y(k)=(y_centro-ymin)+r_vertice*sin(alpha*(k-1)+theta);
        end
        ...
        if rem(i,2)==0
            ...
            elseif i==suddy
                if j==1
                    x=[xmin xmin x(4) x(end)];
                    y=[y(1) ymin ymin y(end)];
                elseif j==suddx
                    x=[x(1) x(2) xmax xmax];
                    y=[y(1) ymin ymin y(end)];
                else
            ...
            elseif i==suddy
                if j==1
                    x=[x(1) xmin x(end) x(end)];
                    y=[y(1) ymin ymin y(end)];
                elseif j==suddx
                    x=[xmax x(2) x(2) xmax];
                    y=[y(1) y(2) ymin ymin];
                else
                    x=[x(1) x(2) x(2) x(end) x(end)];
